<script lang="ts">
  ...
  const statusColors: Record<string, string> = {
    ...
    Drying: 'bg-blue-100 text-pink-500'
  };
...
<div class="min-h-screen bg-[#FFF5F9] pb-24 px-4 pt-6 font-sans">
  <div class="max-w-lg mx-auto">
    ...
    <header class="flex justify-between items-center mb-6">
      <div>
        <h1 class="text-4xl font-black text-[#1A1C1E] tracking-tight">My Wardrobe</h1>
        <p class="text-gray-400 mt-1 text-sm">Your collection</p>
      </div>
      <a href="/wardrobe/add" class="text-white px-4 py-2.5 rounded-2xl font-bold text-sm transition-all active:scale-95 flex-shrink-0" style="background: linear-gradient(135deg, #f472b6, #ec4899); box-shadow: 0 4px 14px rgba(236,72,153,0.3);">
        + Add
      </a>
    </header>
    ...
    {:else if items.length === 0}
      <div class="flex flex-col items-center justify-center py-32 bg-white rounded-[32px] border border-dashed border-gray-200 shadow-sm">
        ...
        <a href="/wardrobe/add" class="mt-8 text-pink-500 font-semibold hover:underline">Add your first item</a>
      </div>
        ...
        {#each filteredItems as item}
          <div class="group bg-white rounded-[28px] overflow-hidden shadow-sm border border-gray-50 hover:shadow-xl transition-all duration-300">
            ...
            <div class="aspect-[3/4] bg-[#F1F4F9] flex items-center justify-center transition-transform duration-500 relative overflow-hidden">
              ...
              <a href="/wardrobe/edit/{item.id}" class="absolute top-3 left-3 opacity-0 group-hover:opacity-100 transition-opacity bg-white/90 backdrop-blur-md text-pink-400 hover:bg-pink-50 w-8 h-8 rounded-full flex items-center justify-center shadow-sm">
            ...
            <div class="p-3">
              <h3 class="text-sm font-bold text-gray-900 mb-1 truncate">{item.name}</h3>
